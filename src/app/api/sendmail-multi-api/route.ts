...
export async function POST(request: NextRequest, response: NextResponse) {
  ...
  try {
    const mail = await transporter.sendMail({
      ...
      // note: This is the plaintext version of the message as an Unicode string, Buffer, Stream or an attachment-like object ({path: ‘/var/data/…'})
      html: `
        <!--
        <p>Name: ${name} </p>
        <p>Email: ${email} </p>
        ...
        <p>Message: ${message} </p>
        `,
      // note: The HTML version of the message as an Unicode string, Buffer
      cc: ccemail,
      bcc: bccemail,
