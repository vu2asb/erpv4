...
const Emailer = () => {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.status == 200) {
        // alert("SC-100:: Your mail has been sent");
  ...
  return (
    <main className="flex min-h-screen flex-col items-center">
      ...
      >
        <div className="m-1 p-1 flex flex-col w-full border border-cyan-500">
          ...
          <input
            ...
            maxLength={50}
            name="name"
            className="text-black border border-cyan-950 bg-slate-200"
          />
          ...
          <textarea
            ...
            rows={1}
            className="text-black  border-cyan-950 bg-slate-200"
          />
          <button className="text-0.8xl rounded bg-blue-900 mt-1" type="submit">
            Send
          </button>
        </div>
      </form>
